  ...
  return (
    <header className="bg-white dark:bg-slate-800 border-b border-gray-100 dark:border-gray-900 z-50">
      <nav className="mx-auto max-w-7xl flex items-center justify-between p-4 lg:px-8">
        {/* Logo image */}
        <Link to="/" className="flex items-center gap-3 hover:opacity-70 custom-transition">
          <img src={logo} alt="Logo" className="w-10 h-10" />
          {/* Show dark mode logo if dark mode is enabled */}
          <span className="w-auto h-6 sidebar-text">PPNS</span>
        </Link>
        {/* Log in and Sign Up Links and Dark Mode Toggle */}
        <div className="flex items-center gap-2 sm:gap-4 font-bold">
          {/* Toggle dark mode */}
          <button type="button" className="hover:opacity-70" onClick={() => dispatch(TOGGLE_DARKMODE())}>
            {darkMode ? <HiOutlineSun className="w-6 h-6" /> : <HiOutlineMoon className="w-6 h-6" />}
          </button>
          {/* <Link to="/login" className="py-2 px-4 hover:bg-gray-200/75 dark:hover:bg-slate-700 rounded-xl">
            Log in
          </Link> */}
          <Link to="/login" className="py-2 px-4 text-white bg-primary hover:bg-primary-shade rounded-xl">
            Login
          </Link>
        </div>
      </nav>
