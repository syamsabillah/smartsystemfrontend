...
const Sidebar = () => {
  ...
  return (
    <aside id="sidebar" className="hidden sm:flex flex-col items-center xl:items-start fixed z-30 w-full h-full min-h-full max-w-[88px] xl:max-w-[300px] p-6 px-4 bg-white dark:bg-slate-800 border-r-2 border-gray-300 dark:border-gray-700">
      {/* Logo */}
      <NavLink className="w-fit xl:px-4 flex items-center gap-3 hover:opacity-70">
        <img src={logo} alt="Logo" className="w-10 h-10" />
        {/* Show dark mode logo if dark mode is enabled */}
        <span className="w-auto h-6 sidebar-text text-xl font-bold">PPNS</span>
      </NavLink>
      ...
      <nav className="w-full mt-8 flex flex-col justify-between gap-2">
        ...
        {/* Logout Button */}
        <button type="button" onClick={() => Auth.logout()} className="sidebar-btn">
